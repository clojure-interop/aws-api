(ns com.amazonaws.retry.PredefinedBackoffStrategies$EqualJitterBackoffStrategy
  (:refer-clojure :only [require comment defn ->])
  (:import [com.amazonaws.retry PredefinedBackoffStrategies$EqualJitterBackoffStrategy]))

(defn ->equal-jitter-backoff-strategy
  "Constructor.

  base-delay - `int`
  max-backoff-time - `int`"
  (^PredefinedBackoffStrategies$EqualJitterBackoffStrategy [^Integer base-delay ^Integer max-backoff-time]
    (new PredefinedBackoffStrategies$EqualJitterBackoffStrategy base-delay max-backoff-time)))
...
(defn compute-delay-before-next-retry
  ...
  (^Long [^PredefinedBackoffStrategies$EqualJitterBackoffStrategy this ^com.amazonaws.retry.v2.RetryPolicyContext context]
...
(defn delay-before-next-retry
  ...
  (^Long [^PredefinedBackoffStrategies$EqualJitterBackoffStrategy this ^com.amazonaws.AmazonWebServiceRequest original-request ^com.amazonaws.AmazonClientException exception ^Integer retries-attempted]
